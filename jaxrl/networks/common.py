...
#         # post_gradient_update = jax.jit(self.pruner.post_gradient_update)
...
#         new_params_head = self.pruner.post_gradient_update(new_params_head, new_opt_state_head)
...
# to separate opt_state for LinearProjector and other layers
@flax.struct.dataclass
class ModelDecoupleOptforLP:
    step: int
    apply_fn: nn.Module = flax.struct.field(pytree_node=False)
    params: Params
    tx: Optional[optax.GradientTransformation] = flax.struct.field(
    ...
    def create(cls,
               ...
               inputs: Sequence[jnp.ndarray],
               tx: Optional[optax.GradientTransformation] = None,
               tx_critic: Optional[optax.GradientTransformation] = None) -> 'ModelDecoupleOptforLP':
        variables = model_def.init(*inputs)
        _, params = variables.pop('params')

        if tx is not None:
            if tx_critic is None:
                tx_critic = tx
            params_critic, _ = split_tree(params, 'reused_critic')
            opt_state = tx_critic.init(params_critic)
            # opt_state_lp = tx.init(params_lp) # here the head refers to Linear Projector
        else:
            opt_state_enc, opt_state_head = None, None
    ...
    def apply_gradient(self, loss_fn) -> Tuple[Any, 'Model']:
        ...
        params_critic, params_lp = split_tree(self.params, 'reused_critic')
        grads_critic, _ = split_tree(grads, 'reused_critic')

        updates_critic, new_opt_state = self.tx_critic.update(grads_critic, self.opt_state,
                                                            params_critic)
        new_params_critic = optax.apply_updates(params_critic, updates_critic)

        # updates_head, new_opt_state_head = self.tx.update(grads_head, self.opt_state_head,
        #                                                   params_head)
        # new_params_head = optax.apply_updates(params_head, updates_head)

        new_params = flax.core.FrozenDict({**params_lp, 'reused_critic': new_params_critic}) # NOTE (ZW) we don't update the Linear Projector
        return self.replace(step=self.step + 1,
                            params=new_params,
                            opt_state=new_opt_state), info
